...
def new_food_category():
    ...
    db.session.commit()
    return jsonify({'message': 'New food category created successfully', 'data':new_food_category}),201


#get,edit and delete food category by id
@categories.route('/food_category/<int:id>', mmethods =['GET','PUT','DELETE'])
def handle_food_category(id):
    ...
    if request.method =='GET':
        response ={
            'id':food_category.id,
            'name':food_category.created_by,
            'created_by':food_category.created_by,
            'created_at':food_category.created_at

        }
        return{'success': True,'category':response,'message':'Food categoruy details retrieved'}
