...
from flask_jwt_extended import get_wt_identity
...
def handle_address(id):
    ...
    if request.method =='GET':
        ...
        return{'sucess': True, 'address':response,'message':'Address details retrieved'}
    elif request.method =='PUT':
        data=request.get_json()

        if not data['name']:
            return jsonify({'message':'address region name is requird'})
        if not data['district_id']:
            return jsonify({'message':'address region name is required'})
        
        address.name =data['name']
        address.district_id =data['district_id']
        address.user_id =get_wt_identity()
        address.updated_at =datetime.utcnow()
        db.session.add(address)
        db.session.commit()
        return {'mesaage':f'{address.name} address updated successfully'}
    elif request.method =='DELETE':
        db.session.delete(address)
        db.session.commit()
        return {'mesaage': f'{address.name} address successfully deleted.'}